...
export function RegisterForm() {
  ...
  const onSubmit = async (data: RegisterFormData) => {
    ...
    try {
      ...
      handleError(error, 'فشل إنشاء الحساب');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    <div className="space-y-4 w-full max-w-sm">
      <div className="space-y-2">
        <Label htmlFor="name" className="text-[#f0f2f8]">الاسم الكامل</Label>
        <Input 
          id="name" 
          {...register('name')} 
          disabled={isLoading}
      ...
      <div className="space-y-2">
        <Label htmlFor="phone" className="text-[#f0f2f8]">رقم الهاتف (اختياري)</Label>
        <Input 
          id="phone" 
          type="tel" 
          dir="ltr"
      ...
      <Button type="button" onClick={handleSubmit(onSubmit)} className="w-full bg-[#0d7e8a] hover:bg-[#0a6b75] text-white transition-colors" disabled={isLoading}>
        {isLoading ? 'جاري التحميل...' : t('register')}
      </Button>
